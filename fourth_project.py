...
for rank, acct_chr in enumerate(list_of_acctchr):
	...
	request+=1
	dbg.write(f"Request iteration {request} for https://www.pathofexile.com/character-window/get-items?accountName={chr}&character={acct} with response code {response.status_code}")
	if(request>=5):
		time.sleep(2)
		request=0
